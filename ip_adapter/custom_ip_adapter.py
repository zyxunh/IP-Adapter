...
class IPAdapterPlus(CustomIPAdapter):
    ...
    def process_pick_mask(self, embeds):
        embeds_pool = embeds[:, :1]
        embeds = embeds[:, 1:].view(1, 16, 16, -1)
        mask = torch.from_numpy(GLOBAL_ITEM.mask).cuda().float()
        mask = F.interpolate(mask[None, None], (16, 16), mode="nearest-exact")[0, 0]
        embeds = embeds[mask[None] >0][None]
        embeds = torch.cat([embeds, embeds_pool], dim=1)
        return embeds
